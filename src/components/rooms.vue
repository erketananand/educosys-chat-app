        ...
        <div
            ...
            <table v-if="rooms.length > 0" class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th scope="col"
                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Room Name
                        </th>
                        <th scope="col"
                        ...
                        <th scope="col"
                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Online
                            Users</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="room in rooms" :key="room.name">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="ml-4">
                                    <div class="text-sm font-medium text-green-800">{{ room.users }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-center text-sm font-medium">
                            <button @click="joinRoom(room)" :disabled="room.status != 'open'"
                                :class="room.status != 'open' ? 'text-gray-600 hover:text-gray-600 cursor-not-allowed' : 'text-indigo-600 hover:text-indigo-900 '">Join
                                Room</button>
                        </td>
                    </tr>
                </tbody>
...
const rooms: Ref<IRoom[]> = ref([] as IRoom[]);
...
async function getRooms() {
    await $io.emit('getRooms', (res) => {
        rooms.value = res.data;
    });

    await $io.on('rooms', (res) => {
        rooms.value = res;
    });
}
